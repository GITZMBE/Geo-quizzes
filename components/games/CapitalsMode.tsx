...
export function CapitalsMode({
  ...
  const capitals = countries.map((c) => c.properties.capital);
  ...
  function guess(answer: string) {
    submitGuess(answer, !!targetCountry && answer.toLowerCase() === targetCountry.properties.capital.toLowerCase());
    setInput("");
  }
      ...
      {!state.finished ? (
        <>
          ...
          >
            {!state.lastResult ? (
              <>
                What is the capital of <span className="font-bold">{target}</span>?
              ...
              <span>
                ...
                <span className="font-bold">{target}</span>&apos;s capital is{" "}
                <span className="font-bold">{targetCountry?.properties.capital}</span>
              </span>
            )}
          </div>
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-6">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Capitals correctly identified</p>
          </div>
          <button
            onClick={playAgain}
